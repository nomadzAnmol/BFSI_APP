...
import girlImage from '../../assets/three.png'; 

const Thirdpage = ({ navigation, route }) => {
  const { activeDotIndex } = route.params; // Get activeDotIndex from params

  const handleNextPress = () => {
    navigation.navigate('FourthPage', { activeDotIndex: 3 }); // Pass activeDotIndex for the fourth page
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.sliderContainer}>
        {[...Array(5)].map((_, index) => (
          <View
            key={index}
            style={index === 2 ? styles.sliderDotActive : styles.sliderDot} // Third dot is active
          />
        ))}
      </View>
