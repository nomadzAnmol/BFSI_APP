...
const ForgetOtpVerify = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchToken = async () => {
      const userToken = await AsyncStorage.getItem('authToken'); // Retrieve the tokena wait AsyncStorage.setItem('authToken', result.token);
...
const handleOtpSubmit = async () => {
  ...
  try {
    const response = await fetch(`${API_URL}/forgotPasswordVerify`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok && result.status === "true") {
      ...
      setTimeout(() => {
        hideDialog();
        navigation.navigate('ResetPassword'); // Navigate to the LogIn screen
      }, 1000);
    } else {
